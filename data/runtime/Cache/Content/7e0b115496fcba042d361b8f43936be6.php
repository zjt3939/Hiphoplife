...
<body class="J_scroll_fixed">
	<div class="wrap J_check_wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="javascript:;">所有音乐</a></li>
			<li><a href="<?php echo U('AdminMusicManage/add',array('term'=>empty($term['term_id'])?'':$term['term_id']));?>" target="_self">添加音乐</a></li>
		</ul>
		<form class="well form-search" method="post" action="<?php echo U('AdminMusicManage/index');?>">
			<div class="search_type cc mb10">
				<div class="mb10">
					<span class="mr20">音乐类型：
						<select name="category_id" id="category_id">
							<option value=''>全部</option>
							<?php if(is_array($MusicCategory)): foreach($MusicCategory as $key=>$vo): if(!empty($selectData['category_id'])){ $selected = $vo['category_id']==$selectData['category_id']?'selected':''; } ?>
		...
		<form class="J_ajaxForm" action="" method="post">
			<table class="table table-hover table-bordered table-list">
				...
				<?php if(is_array($MusicData)): foreach($MusicData as $key=>$do): ?><tr>
						...
						<td>
						<a href="<?php echo U('AdminMusicManage/edit',array('id'=>$do['id']));?>">修改</a> | 
						<a href="<?php echo U('AdminMusicManage/delete',array('id'=>$do['id']));?>" class="J_ajax_del">删除</a></td>
	...
	<script>
		...
		function refersh_window() {
			...
			if (refersh_time == 1) {
				window.location = "<?php echo U('AdminMusicManage/index',$formget);?>";
			}
		}
