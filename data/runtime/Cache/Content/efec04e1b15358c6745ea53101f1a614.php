...
<div class="wrap J_check_wrap">
  <ul class="nav nav-tabs">
     ...
     <li><a href="<?php echo U('AdminVideoManage/add');?>">添加视频</a></li>
     <li class="active"><a href="javascript:void(0)">编辑视频</a></li>
  </ul>
  <form name="myform" id="myform"  action="<?php echo U('AdminVideoManage/edit_post');?>" method="post" class="form-horizontal J_ajaxForms" enctype="multipart/form-data">
  <div class="col-auto">
    <div class="table_full">
      <table width="100%" cellpadding="2" cellspacing="2">
      			...
      			<td width="150px">
      				<select name="category_id">
      					<option value="">请选择视频类型</option>
							<?php if(is_array($VideoCategory)): foreach($VideoCategory as $key=>$vo): $cSelected =$vo['category_id']==$editVideoData['category_id']?"selected":''; ?>
      		...
      		<tr>
      			...
      			<td width="150px">
      				<select name="gather_id">
						...
						<?php if(is_array($videoGather)): foreach($videoGather as $key=>$go): $gSelected =$go['gather_id']==$editVideoData['gather_id']?"selected":''; ?>
            ...
            <tr>
              ...
              <td>
              <td>
              	<input type="text"  name="name" id="name" value="<?php echo ($editVideoData['name']); ?>" style="color:" class="input input_hd J_title_color" placeholder="请输入视频名称" onkeyup="strlen_verify(this, 'title_len', 160)" />
              	<span class="must_red">*</span>
              </td>
            </tr>
            <tr>
              <th width="80">关键词</th>
              <td><input type='text' name='keywords' id='keywords' value='<?php echo ($editVideoData['keywords']); ?>'    class='input' placeholder='请输入关键字'> 多关键词之间用空格隔开</td>
            </tr>
            <tr>
            	<th>url</th>
            	<td><input type="text" name="url" id="url" placeholder='请输入url' value="<?php echo ($editVideoData['url']); ?>">*不能为空</td>
            </tr>
            <tr>
            	<th>来源</th>
            	<td><input type="text" name="source" id="source" placeholder='请输入来源' value="<?php echo ($editVideoData['source']); ?>" ></td>
            </tr>
            <tr>
              <th width="80">详细介绍</th>
              <td><textarea name='detail' id='detail' value="<?php echo ($editVideoData['detail']); ?>" style='width:50%;height:200px;'  ><?php echo ($editVideoData['detail']); ?></textarea> </td>
            </tr>
	        <tr>
	        	<th width="80">缩略图</th>
	          <td>
  ...
  <div class="form-actions">
  		<input type="hidden" name="Id" value="<?php echo ($editVideoData['id']); ?>">
...
$(function () {
	 Wind.use('validate', 'ajaxForm', 'artDialog', function () {
	        ...
	        form.validate({
	            ...
	            //验证未通过提示消息
	            messages: {'name':{required:'视频名称不能为空'},'category_id':{required:'视频类型不能为空'},'gather_id':{required:'视频专辑不能为空'},'url':{required:'视频url不能为空'},'keywords':{required:'关键字不能为空'}},
	            //给未通过验证的元素加效果,闪烁等
	            highlight: false,
	            //是否在获取焦点时验证
	            onfocusout: false,
