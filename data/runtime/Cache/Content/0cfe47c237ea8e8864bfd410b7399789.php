...
<div class="wrap J_check_wrap">
  <ul class="nav nav-tabs">
     <li><a href="<?php echo U('AdminGatherManage/index');?>">所有专辑</a></li>
     <li class="active"><a href="<?php echo U('AdminGatherManage/add');?>">添加专辑</a></li>
  </ul>
  <form name="myform" id="myform"  action="<?php echo U('AdminGatherManage/add_post');?>" method="post" class="form-horizontal J_ajaxForms" enctype="multipart/form-data">
  <div class="col-auto">
      			...
      			<td width="150px">
      				<select name="category_id">
      					<option value="">请选择视频类型</option>
							<?php if(is_array($VideoCateGoryData)): foreach($VideoCateGoryData as $key=>$vo): ?><option value="<?php echo ($vo['category_id']); ?>"><?php echo ($vo['category_name']); ?></option><?php endforeach; endif; ?>
      				</select>*不能为空
      			</td>
      		</tr>
            <tr>
              <th width="80">专辑名称</th>
              <td>
              	<input type="text"  name="gather_name" id="name" value="" style="color:" class="input input_hd J_title_color" placeholder="请输入专辑名称" onkeyup="strlen_verify(this, 'title_len', 160)" />
  ...
  <div class="form-actions">
        ...
        <a class="btn" href="<?php echo U('AdminGatherManage/index');?>">返回</a>
  </div>
 </form>
</div>
	...
	$('.file').hide();
	 Wind.use('validate', 'ajaxForm', 'artDialog', function () {
	        ...
	        form.validate({
	            ...
	            //验证未通过提示消息
	            messages: {'name':{required:'专辑名称不能为空'},'category_id':{required:'专辑类型不能为空'},'gather_id':{required:'专辑专辑不能为空'},'keywords':{required:'关键字不能为空'}},
	            //给未通过验证的元素加效果,闪烁等
	            highlight: false,
	            //是否在获取焦点时验证
	            onfocusout: false,
										...
										button:[
											{
												...
												name: '返回列表',
												callback:function(){
													location.href="<?php echo U('AdminGatherManage/index');?>";
													return true;
												}
											}
										]
