...
<div class="wrap J_check_wrap">
  <ul class="nav nav-tabs">
     ...
     <li class="active"><a href="<?php echo U('AdminMusicManage/add');?>">添加音乐</a></li>
  </ul>
  <form name="myform" id="myform"  action="<?php echo U('AdminMusicManage/add_post');?>" method="post" class="form-horizontal J_ajaxForms" enctype="multipart/form-data">
  <div class="col-auto">
    <div class="table_full">
      ...
      <table width="100%" cellpadding="2" cellspacing="2">
      			...
      			<td width="150px">
      				<select name="category_id">
      					<option value="">请选择音乐类型</option>
							<?php if(is_array($MusicCategory)): foreach($MusicCategory as $key=>$vo): ?><option value="<?php echo ($vo['category_id']); ?>"><?php echo ($vo['category_name']); ?></option><?php endforeach; endif; ?>
      				</select>*不能为空
      			</td>
      		</tr>
      		<tr>
      			...
      			<td width="150px">
      				<select name="gather_id">
						...
						<?php if(is_array($musicGather)): foreach($musicGather as $key=>$go): ?><option value="<?php echo ($go['gather_id']); ?>"><?php echo ($go['gather_name']); ?></option><?php endforeach; endif; ?>
      				</select>*不能为空
      			</td>
      		</tr>
            <tr>
              <th width="80">音乐名称</th>
              <td>
              	<input type="text"  name="name" id="name" value="" style="color:" class="input input_hd J_title_color" placeholder="请输入音乐名称" onkeyup="strlen_verify(this, 'title_len', 160)" />
            ...
            <tr>
             ...
             <tr class="url">
            	<th>url</th>
            	<td><input type="text" name="url" id="url" placeholder='网络音乐请输入url' >*不能为空</td>
            </tr>
	        <tr>
	        	<th width="80">缩略图</th>
	          <td>
	...
	$('.file').hide();
	 Wind.use('validate', 'ajaxForm', 'artDialog', function () {
	        ...
	        form.validate({
	            ...
	            //验证未通过提示消息
	            messages: {'name':{required:'音乐名称不能为空'},'category_id':{required:'音乐类型不能为空'},'gather_id':{required:'音乐专辑不能为空'},'keywords':{required:'关键字不能为空'}},
	            //给未通过验证的元素加效果,闪烁等
	            highlight: false,
	            //是否在获取焦点时验证
	            onfocusout: false,
